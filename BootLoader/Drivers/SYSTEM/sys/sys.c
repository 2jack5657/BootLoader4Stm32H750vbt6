...
uint8_t sys_stm32_clock_init(uint32_t plln, uint32_t pllm, uint32_t pllp, uint32_t pllq)
{
    ...
    MODIFY_REG(PWR->CR3, PWR_CR3_SCUEN, 0);                         /* SCUEN = 0, 锁定LDOEN和BYPASS位的设置 */
    __HAL_PWR_VOLTAGESCALING_CONFIG(PWR_REGULATOR_VOLTAGE_SCALE0);  /* VOS = 0, Scale0, 1.35V内核电压，使用最高主频480MHz */
    while(!__HAL_PWR_GET_FLAG(PWR_FLAG_VOSRDY)) {}                  /* 等待电压稳定 */

    /* 使能HSE，并选择HSE作为PLL时钟源，配置PLL1，开启USB时钟 */
    rcc_osc_init.OscillatorType = RCC_OSCILLATORTYPE_HSE | RCC_OSCILLATORTYPE_HSI48;
     ...
     */
    rcc_periph_clk_init.PeriphClockSelection = RCC_PERIPHCLK_USART2 | RCC_PERIPHCLK_USART1
    ...
    rcc_periph_clk_init.QspiClockSelection = RCC_QSPICLKSOURCE_D1HCLK;
    rcc_periph_clk_init.Usart234578ClockSelection = RCC_USART234578CLKSOURCE_D2PCLK1;
